...
(function () {
  ...
  ProductGridItem.prototype.compileTemplate = function (data) {
    ...
    itemHtml = itemHtml.replace(/{{itemTag}}/g, buildTagLabel(data, "itemTag"));
    itemHtml = itemHtml.replace(/{{itemTaghandle}}/g, buildTagLabel(data).toLocaleLowerCase().replaceAll(' ', '-'));
    // Add Images
  ...
  function buildTagLabel(data, type="") {
    var saleLabel = '';
    for (let i = 0; i < data.tags.length; i++) {
      const tag = data.tags[i];
      if (tag.indexOf('_label:') > -1) {
        saleLabel = tag.replace('_label:', '')
      }
    }
    if(saleLabel == "New" && type == "itemTag") saleLabel = `<span class="badge badge--top-right new color-accent-1">
                  New
                </span>`
    return saleLabel;
  }
  ...
  function buildReview(data) {
    var html = '';
    // html = `<div class="pwr-category-snippets" style="">
    //         <div id="snippet-${data.id}"></div>
    //       </div>`;
    html = `<div id="yotpo-category-stars">
              <div class="yotpo-widget-instance" data-yotpo-instance-id="851116" data-yotpo-product-id="${data.id}" data-yotpo-cart-product-id="${data.id}" data-yotpo-section-id="collection"></div>
          </div>`
    return html;
  }
